/*
 *
 * JMeta - Meta's java implementation
 *
 * Copyright (C) 2013-2015 Pablo Joubert
 * Copyright (C) 2013-2015 Thomas Lavocat
 * Copyright (C) 2013-2015 Nicolas Michon
 *
 * This file is part of JMeta.
 *
 * JMeta is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * JMeta is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 *
 */
package org.meta.model;

import org.meta.api.storage.MetaCache;
import org.meta.executors.MetaScheduledTask;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 *
 * Task running periodically to force the removal of expired entries from {@link MetaCache}.
 *
 * Also force synchronization of cache to storage.
 *
 * @author dyslesiq
 * @version $Id: $
 */
public class StorageExpirationTask extends MetaScheduledTask {

    private final Logger logger = LoggerFactory.getLogger(StorageExpirationTask.class);

    /**
     * Wait for 1 minute before execution.
     */
    public static final long EXECUTION_DELAY = 60;

    /**
     * Executes every minutes.
     */
    public static final long EXECUTION_PERIOD = 60;

    private final MetaCache cache;

    /**
     * <p>Constructor for StorageExpirationTask.</p>
     *
     * @param cacheStorage the cache storage
     */
    public StorageExpirationTask(final MetaCache cacheStorage) {
        super(EXECUTION_DELAY, EXECUTION_PERIOD);
        cache = cacheStorage;
    }

    /** {@inheritDoc} */
    @Override
    public void run() {
        logger.debug("Removing expired entries from cache...");
        cache.removeExpiredEntries();
        logger.debug("Forcing synchronization of cache to storage...");
        cache.flushToStorage();
    }

}
